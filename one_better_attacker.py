...
def move(bot, state):
    ...
    if (target is None) or (len(cluster) == 0):
        ...
        if is_stuck:
            cluster = cluster_list.pop(random.randint(0,len(cluster_list)-1))
        else:
            if len(cluster_list[0]) == 1:
                f_dist_list = []
                for single_food in cluster_list:
                    f_path = nx.shortest_path(state['graph'], bot.position, single_food[0])
                    f_dist = len(f_path)
                    f_dist_list.append(f_dist)
                food_ix = np.argmin(f_dist_list)
